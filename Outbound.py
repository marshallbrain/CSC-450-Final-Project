...
class Outbound:
    ...
    def __init__(self, main_frame, db):
        ...
        table.heading('timestamp', text='Shipping Date', command=lambda: self.sort_table("timestamp", False))
    ...
    def add_item(self, window, name, item, amount, date):
        timestamp = datetime.strptime(date, "%m/%d/%y").timestamp()
        self.db.put_outbound(name, amount, item, timestamp)
        self.update_table()
        window.destroy()

    def update_table(self):
        rows = utility.update_table(self.db, "outbound", self.col, self.ascending, self.search)
    ...
    def select_item(self, event):
        ...
        if item != "":
            ...
            submit = ttk.Button(info, text="Order Shipped", command=lambda: self.arrived(info, item[0]))
            submit.pack(pady=10)

    def arrived(self, window, prod_id):
        self.db.shipped(prod_id)
        window.destroy()
        self.update_table()
